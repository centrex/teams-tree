...
wp core install \
	--path=$LANDO_WEBROOT \
	--url="http://teamscentrex.lndo.site" \
	'--title="Let’s Create some teams"' \
	--admin_user="teams" \
	--admin_password="teams" \
	--admin_email="teams@example.org" \
	--skip-email

# Link our plugin
ln -sF $LANDO_MOUNT $LANDO_WEBROOT/wp-content/plugins/teams
wp plugin activate teams --path=$LANDO_WEBROOT
